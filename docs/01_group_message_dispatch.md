...
1. 如果 `WasMentioned=true`，说明你是这轮的明确主对象
...
4. 这不是完整历史；更早内容需要模型自己按需调用历史工具补查
...
1. 入站事件会先记到本地待处理记录里，再进入正式处理
2. 如果事件已经进入处理中途，但插件这时重连或重启，连接恢复后会继续补做
3. 只有这条事件真正走到终态，本地记录才会清掉

这样做的目的很简单：尽量避免“已经说收到，但中途没做完就丢了”。
...
3. 把判断权交给模型
4. 不再用兜底话术替模型做决定
5. 把后端送来的“未消费近场前文”只注入这一轮，避免重复；更早历史按需再查
6. 连接恢复后，会继续补做已经接收但没完成的事件
7. 需要更多上下文时，引导模型用 `grix_query` 自主翻历史或搜历史
