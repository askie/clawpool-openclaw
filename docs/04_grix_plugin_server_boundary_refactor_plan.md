...
3. `grix_query`、`grix_group`、`grix_admin` 这类本质上偏远端管理的能力，也放进了客户机插件
...
为了保证两侧协议长期干净整洁，后续实现必须再加四条硬规则：

1. 一项能力只能有一个正式入口，不能一半走协议字段、一半走聊天文本
2. 插件不能补 server 应负责的策略判断，server 也不能依赖插件私有实现细节来定义协议
3. 责任边界先定，再定字段；不能先在一侧临时落实现，事后再倒推协议
4. 文档必须区分“责任归属”“协议定义”“当前实现”，不能混写

这四条规则的直接含义是：

1. 远端业务统一走 `agent_invoke`
2. 本地宿主动作统一走 `local_action`
3. 普通聊天统一走 `event_msg` / `send_msg`
4. 媒体、表情、线程、投票这类传输能力统一走稳定合同字段和命令

任何后续需求，只要需要同时改两侧，都必须先检查是不是违反了这四条规则。
...
3. 插件不可以定义业务含义
4. 插件不可以定义降级策略
5. 插件不可以决定“这次该走哪种业务流程”

为了避免边界被说散，插件侧可以直接按这张表自查：

| 看到的需求 | 插件能不能接 | 判断标准 |
|---|---|---|
| 新增稳定字段映射 | 可以 | 只是在 AIBot 稳定字段和 OpenClaw 稳定字段之间搬运 |
| 新增本地动作执行 | 可以 | 动作由 server 明确定义，插件只负责执行和回执 |
| 新增远端业务流程 | 不可以 | 这类需求应该回到 server 编排 |
| 新增降级兼容判断 | 不可以 | 这类判断应该回到 server 能力协商 |
| 想用聊天文本承载协议 | 不可以 | 说明入口设计已经不干净，需要回到合同重定 |
...
1. server 可以决定协议能力是否开启
2. server 可以决定能力如何降级
3. server 可以决定业务动作怎么编排
4. server 不应该把这些判断回推给插件
...
1. 不能让 server 依赖 OpenClaw 私有本地状态细节来完成协议定义
2. 不能把本地宿主控制重新塞回普通聊天文本，逼插件做文本解析
...
| 远端管理工具 | `src/admin/*` | Server | 已收口到 `grix_query` / `grix_group` / `grix_admin` + 本地 `doctor`，但仍属于 pending-migration 🔶 |
...
| Agent 创建管理 | `src/skill-tools/grix-admin-tool.ts` + `src/admin/agent-admin-service.ts` | 插件→WS | 统一收口到 `grix_admin` 单一入口；不再保留独立 create-agent 工具或 CLI ✅ |
...
### 阶段 3：缩减插件公开能力面，收口 agent_api_create 到 grix_admin ✅ DONE

这一阶段要开始收口：

1. 逐步弱化 `src/admin/*` 这类远端管理型入口 ✅（独立的 create-agent 入口已删除，外部统一走 `grix_admin`）
2. `agent_api_create`（创建 agent）不再暴露独立工具或 CLI，而是作为 `grix_admin` 的受 scope 保护能力触发 ✅
3. README 和安装文档里，强调 server 侧适配仍是主路径；在已有主通道与 scope 时，远端创建统一交给 `grix_admin` ✅
