...
test("resolveAgentAPIBase throws when apiBaseUrl and wsUrl are both missing", () => {
  assert.throws(
    () =>
      resolveAgentAPIBase(
        buildAccount({
...
test("resolveAgentAPIBase uses ws url when apiBaseUrl is empty", () => {
  const base = resolveAgentAPIBase(
    buildAccount({
      wsUrl: "wss://grix.dhf.pub/v1/agent-api/ws?agent_id=123",
    }),
...
test("callAgentAPI sends bearer request and parses success payload", async (t) => {
  ...
  assert.match(infoLogs[0] ?? "", /agent=9992/);
  ...
  assert.match(infoLogs[0] ?? "", /timeout_ms=15000/);
  assert.match(infoLogs[0] ?? "", /api_key=present/);
  assert.match(infoLogs[0] ?? "", /query_keys=none/);
  assert.match(infoLogs[0] ?? "", /body_keys=name/);
  assert.match(infoLogs[0] ?? "", /body_bytes=19/);
...
test("callAgentAPI reports biz error with status and code", async (t) => {
  ...
  assert.match(errorLogs[0] ?? "", /agent=9992/);
  ...
  assert.match(errorLogs[0] ?? "", /timeout_ms=15000/);
  assert.match(errorLogs[0] ?? "", /api_key=present/);
  assert.match(errorLogs[0] ?? "", /query_keys=none/);
  assert.match(errorLogs[0] ?? "", /body_keys=name/);
  assert.match(errorLogs[0] ?? "", /body_bytes=19/);
  assert.match(errorLogs[0] ?? "", /status=403 code=20011 msg=agent scope forbidden/);
  assert.match(errorLogs[0] ?? "", /has_data=false/);
});
