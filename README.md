...
The binding command above can refresh the binding itself, but the `openclaw config set` changes in the manual steps still require a Gateway restart before they fully take effect:
...
- Any setting written through `openclaw config set` requires `openclaw gateway restart` before it takes effect.
...
These approval settings are also written through `openclaw config set`, so restart Gateway after saving them:
