...
11. 如果本次安装产出了新的目标 agent，基础指引发完后，必须紧接着明确问一句是否需要立刻拉一个测试群做身份验收；不要在资料卡后直接结束，也不要等用户自己提起。
12. 用户同意拉群测试时，必须由当前主 agent 新建测试群，并把主 agent、当前私聊里的主人、刚安装出的目标 agent 一起拉进群。
13. 测试群建好并拿到准确 `session_id` 后，必须回到当前私聊单独发一张测试群会话卡片，让用户可以直接点进去。
14. 主 agent 随后必须在群里直接 `@` 新 agent，明确问它“你是谁，请介绍一下你自己”或等价问题，不能把验收动作甩给用户先来。
15. 如果新 agent 的回答和 egg 设定的人设、身份、语气或职责明显不一致，不能草草结束；要把它当成安装验收未通过，继续排查并修复，直到重新提问后回答正确为止。
16. 出错时，直接说明失败点、影响和下一步建议，不要模糊带过。
17. 最终明确告诉用户：装到了哪个 agent、结果成功还是失败、后续是否还要继续操作。
...
4. 如果本次安装产出了新的目标 agent，基础指引后必须继续问一句是否要现在拉测试群做身份验收；这句询问属于必做收尾，不是可省略建议。
5. 如果用户还没回答是否拉群，就停在这里等回复；只有用户明确同意，才继续建群验收。

建议直接使用或等价改写下面这句：

```text
可以点开上面的资料卡查看它的资料，也可以从资料页继续和它对话。要不要我现在帮你建一个测试群，当场验收一下它的身份是否正常？
```
...
17. 再发一条普通文字，先完成基础指引；如果 route=`openclaw_create_new`，必须在同一条或下一条普通文字里继续明确问一句：要不要现在就拉一个测试群，帮他当场验收这个新 agent。
18. 如果 route=`openclaw_create_new`，在用户回复前停在当前私聊等待；用户明确拒绝则结束，明确同意再继续。
...
21. 在当前私聊里同步一次测试群已建好；如果已经拿到准确 `session_id`，必须单独发送一张会话卡片帮助用户直接进入测试群。会话卡片一律按 `message-send` 里的 `grix://card/conversation` 协议发送，并且必须单独一条发送；如果还要补说明，另发普通文字。
...
1. `openclaw_create_new` 成功后的最后一句，默认就用建群验收问题收口，不要停在“可以点开资料卡查看”这类说明上。
2. 建议话术：
   - `可以点开上面的资料卡查看它的资料，也可以从资料页继续和它对话。要不要我现在帮你建一个测试群，当场验收一下它的身份是否正常？`
3. 只要用户还没回答要不要拉群，就先等待，不要自顾自结束，也不要直接开始建群。
...
2. 如果创建了测试群且拿到了准确 `session_id`，必须补一张测试群会话卡片，方便用户直接点进去继续聊。发送格式沿用 `message-send` 的 `grix://card/conversation` 规则：卡片必须单独一条发送，不要和说明文字混在一起，也不要在一条消息里塞多张卡片。
